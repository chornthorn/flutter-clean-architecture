...
| `Core/Networking/`                   | `lib/core/networking/`                                             | `NetworkClient` is the one `Dio` every feature's endpoints share, with its timeouts, base URL, logging, and error mapping (the place for auth or retry). Bound in `provider.dart`, so features take it from the container rather than importing this.                                                                                                     |
