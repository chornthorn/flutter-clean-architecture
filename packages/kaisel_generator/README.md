...
- **Unified Package**: Dart annotations (`@KaiselModule`, `@KaiselMicroPackage`, `@KaiselInit`), the Dart CLI runner (`bin/kaisel_generator.dart`), the `build_runner` entry point (`lib/builder.dart`) and the generator itself — grouped by stage in `lib/src/`: `annotations/`, `model/`, `parser/`, `scanner/`, `emitter/`, `generator/`, plus the SPI in `spi/` and its implementations in `provider/`.

### The output is open: SPI, providers, factory

The pipeline is fixed — scan a project, hand what it found to one provider,
deliver what that provider produced — and what gets generated sits behind a
service provider interface:

| Piece                        | Role                                                                                                                                                                     |
| :--------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GenerationProvider`         | The SPI. `supports(request)` claims a project, `generate(request)` returns a `GeneratedOutput`: the run's output file plus any side files the composition needs on disk. |
| `HostRegistryProvider`       | Serves a host application: the module registry, and the manifests of the registered packages that live inside the project.                                               |
| `MicroPackageProvider`       | Serves a micro-package: its manifest, and nothing else.                                                                                                                  |
| `GenerationProviderFactory`  | Creates the built-in providers, with their scanners, parsers and emitters wired — the one place that knows how they are constructed.                                     |
| `GenerationProviderRegistry` | Holds the providers and resolves the first one that supports a request; a project no provider claims fails with a generation error instead of writing nothing.           |

A project that needs another generated file — a shell registry, a route table, a
URL manifest — registers a provider instead of growing the generator:

```dart
final generator = KaiselGenerator(
  providerRegistry: GenerationProviderRegistry.of([
    const MyRouteTableProvider(),
    // ...or keep the built-ins and put yours in front of them.
    ...const GenerationProviderFactory().createProviders(),
  ]),
);
final result = await generator.generate(root: '.');
```

The generator hands a provider a `GenerationRequest`: the project root, the
