...
packages/
  kaisel_generator/                 annotations, generated-code helpers + the Rust generator
```
...
Kaisel runs as a build_runner builder backed by a Rust engine
(`kaisel_generator:kaisel`, enabled in `build.yaml`). build_runner owns the host
registry — the engine returns its source and the builder writes it — while the
manifests are written by the engine itself, because build_runner forbids a
builder writing another package's assets.
