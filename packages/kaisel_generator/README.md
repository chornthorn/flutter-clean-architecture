...
- **Unified Package**: Dart annotations (`@KaiselModule`, `@KaiselMicroPackage`, `@KaiselInit`), the Dart CLI runner (`bin/kaisel_generator.dart`), the `build_runner` entry point (`lib/builder.dart`) and the generator itself — grouped by stage in `lib/src/`: `annotations/`, `model/`, `parser/`, `scanner/`, `emitter/`, `generator/`, the capability SPIs in `spi/`, and their implementations in `provider/`.

### The output is open: SPI, providers, factories

The pipeline is fixed — scan a project, hand what it found to one provider,
deliver what that provider produced — and every stage of it sits behind a service
provider interface. The SPI _framework_ is [`package:spi`](../spi) (a Dart
service-provider-interface standard: `Spi`, `Provider`, `ProviderFactory`,
`ProviderSession`, `ProviderManager`); what is Kaisel's is the set of capabilities:

| SPI                   | Providers                 | Role                                                                                                          |
| :-------------------- | :------------------------ | :------------------------------------------------------------------------------------------------------------ |
| `LibraryScannerSpi`   | `DefaultLibraryScanner`   | Reads what a package declares under `lib/`.                                                                   |
| `ProjectScannerSpi`   | `DefaultProjectScanner`   | Reads `kaisel.yaml`, `pubspec.yaml`, and the package config that resolves imports.                            |
| `AnnotationParserSpi` | `DefaultAnnotationParser` | Reads `@KaiselModule`, `@KaiselInit` and `@KaiselMicroPackage` out of source.                                 |
| `ManifestParserSpi`   | `DefaultManifestParser`   | Reads the mounts a generated manifest declares.                                                               |
| `ImportEmitterSpi`    | `DefaultImportEmitter`    | Writes the import block a generated file opens with.                                                          |
| `RegistryEmitterSpi`  | `DefaultRegistryEmitter`  | Writes the host's registry: routes, mounts, codec, router config.                                             |
| `ManifestEmitterSpi`  | `DefaultManifestEmitter`  | Writes a micro-package's manifest.                                                                            |
| `GenerationSpi`       | `HostRegistryProvider`    | Serves a host application: the module registry, plus the manifests of registered packages inside the project. |
|                       | `MicroPackageProvider`    | Serves a micro-package: its manifest, and nothing else.                                                       |

A project that needs another generated file — a shell registry, a route table, a
URL manifest — registers a provider instead of growing the generator. The entry
point is the composition root, and it is the one place that names implementations:

```dart
// lib/builder.dart of a host that adds its own provider, or of a package that
// ships one to hosts.
Builder openApiBuilder(BuilderOptions options) => kaiselBuilder(
      options,
      generator: KaiselBootstrap(
        providerFactories: [
          // Lower order than defaultProviderOrder, so it is asked first.
          const OpenApiEmitterFactory(),
          ...kaiselProviderFactories,
        ],
      ).createGenerator(),
    );
```

A generation provider is handed a `GenerationRequest`: the project root, the
scanned modules (initial landing module first), the package name, the `kaisel.yaml`
and `@KaiselInit` configuration, the output path, and whether the caller wants the
output written or its source handed back.

The SPI framework is deliberately **not** re-exported from
`package:kaisel_generator`: a host imports this library next to
`package:provider`, and a second `Provider` in the namespace would make every such
...
void main() async {
  final result = await const KaiselBootstrap().createGenerator().generate(
    root: '.',
    force: false,
  );
