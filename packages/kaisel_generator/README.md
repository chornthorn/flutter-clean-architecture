...
- **Unified Package**: Dart annotations (`@KaiselModule`, `@KaiselMicroPackage`, `@KaiselInit`), the Dart CLI runner (`bin/kaisel_generator.dart`), the `build_runner` entry point (`lib/builder.dart`) and the generator itself — grouped in `lib/src/`: `annotation/`, `models/`, `helper/`, and `service/` (one file per stage).
...
`KaiselGenerator` builds every stage itself and hands the run a
`GenerationRequest`: the project root, the scanned modules (initial landing module
first), the package name, the `kaisel.yaml` and `@KaiselInit` configuration, the
output path, and whether the caller wants the output written or its source handed
back. The run then produces a host's registry or a micro-package's manifest, by
`request.isHost`.
