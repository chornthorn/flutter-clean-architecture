...
- **Unified Package**: Dart annotations (`@KaiselModule`, `@KaiselMicroPackage`, `@KaiselInit`), the Dart CLI runner (`bin/kaisel_generator.dart`), the `build_runner` entry point (`lib/builder.dart`) and the generator itself — grouped by stage in `lib/src/`: `annotations/`, `model/`, `parser/`, `scanner/`, `emitter/`, `generator/`.
