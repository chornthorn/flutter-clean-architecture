...
void main() {
  ...
  group('ShopProductViewModel', () {
    test('should load the product the use case returns', () async {
    ...
    test('should start the add settled, so the page does not read it in flight', () {
      final viewModel = shopProductViewModel(MockProductRepository());
      addTearDown(viewModel.dispose);
    ...
    test('should run the write and count what the cart holds', () async {
      final repository = MockProductRepository();
      when(() => repository.productById('sku-42'))
          .thenAnswer((_) async => product);
